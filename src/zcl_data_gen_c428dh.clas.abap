CLASS zcl_data_gen_c428dh DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_data_gen_c428dh IMPLEMENTATION.

  METHOD if_oo_adt_classrun~main.

    out->write( 'Deleting existing data...' ).

    DELETE FROM ztravel_428dh.

    INSERT ztravel_428dh FROM (
    ...
    DELETE FROM zbooking_428dh.

    INSERT zbooking_428dh FROM (

      SELECT
        FROM /dmo/booking
        JOIN ztravel_428dh ON /dmo/booking~travel_id = ztravel_428dh~travel_id
        JOIN /dmo/travel ON /dmo/travel~travel_id = /dmo/booking~travel_id
        FIELDS
          "client,
          uuid( ) AS booking_uuid,
          ztravel_428dh~travel_uuid AS parent_uuid,
          ...
          ztravel_428dh~last_changed_at AS local_last_changed_at ).

    out->write( 'Adding Booking Supplements data' ).

    DELETE FROM zbsuppl_428dh.

    INSERT zbsuppl_428dh FROM (
      SELECT FROM /dmo/book_suppl AS supp
        JOIN ztravel_428dh  AS trvl ON trvl~travel_id = supp~travel_id
        JOIN zbooking_428dh AS book ON book~parent_uuid = trvl~travel_uuid
